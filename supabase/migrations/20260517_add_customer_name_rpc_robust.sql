-- Safely add missing columns to ensure no schema errors
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS customer_name text;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS transaction_id uuid;
...
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS bundle_id uuid;

-- Forcefully drop EVERY version of create_order_atomic to remove all conflicts
DO $$ 
DECLARE 
    r RECORD; 
BEGIN 
...
-- Tell Supabase to refresh its API cache immediately
