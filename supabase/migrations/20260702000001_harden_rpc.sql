-- ============================================================
-- MIGRATION: Harden process_pharmacy_sale RPC
-- Objective: Make sale processing idempotent (to handle offline sync retries)
-- and add server-side cash validation to ensure cash given covers the total.
-- ============================================================
...
DECLARE
  ...
  v_prev_qty  INTEGER;
BEGIN
  -- 🛡️ IDEMPOTENCY CHECK: If order already exists, return its ID safely
  ...
  -- 🛡️ FINANCIAL VALIDATION: Ensure cash covers total for paid cash sales
  IF p_payment_status = 'paid' AND p_intent = 'cash' AND COALESCE(p_cash_given, 0) < p_total THEN
    RAISE EXCEPTION 'INSUFFICIENT_CASH: Cash given (%) must be greater than or equal to total (%)', COALESCE(p_cash_given, 0), p_total;
  END IF;
