...
-- This trigger usually sits on auth.users (outside public), so we create it manually in the dashboard or here if we have permission
-- create trigger on_auth_user_created after insert on auth.users for each row execute procedure public.handle_new_user_restaurant();
...
-- Simple permissive policies for now (tighten before production)
