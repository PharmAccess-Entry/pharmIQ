...
import {
  ...
  BookOpen, Send, Upload, Copy, Trash2, MessageCircle, Loader2, CheckCircle2, Star, BellRing, NavigationOff, MessageSquareText, ChevronDown, ChevronUp, LogOut, Sparkles
} from "lucide-react";
...
const MenuItemCard = memo(({ item, isEvent, onClick }: { item: MenuItem; isEvent: boolean; onClick: () => void }) => {
  ...
  return (
    <div
      ...
      className={`group relative overflow-hidden bg-card hover:bg-accent/5 border border-border rounded-2xl transition-all duration-300 hover:border-primary/30 hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] dark:hover:shadow-[0_8px_30px_rgba(0,0,0,0.4)] active:scale-[0.98] cursor-pointer flex gap-3 p-3.5 ${(!item.available || isOutOfStock) ? "opacity-50 grayscale" : ""}`}
    >
      <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
      
      <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-xl bg-secondary shrink-0 overflow-hidden relative border border-border/50 shadow-inner">
        <Skeleton className="absolute inset-0 z-0 bg-muted" />
        {item.image ? (
          <img src={item.image} alt={item.name} loading="lazy" decoding="async" className="w-full h-full object-cover relative z-10 transition-transform duration-500 group-hover:scale-105" />
        ) : (
      ...
      <div className="flex-1 min-w-0 py-0.5 flex flex-col justify-between relative z-10">
        <div>
          <h3 className="font-bold text-sm sm:text-base leading-tight line-clamp-1 group-hover:text-primary transition-colors text-foreground">{item.name}</h3>
...
const QrMenu = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [menuRes] = await Promise.all([
        ...
        .select("id, name, description, price, category, image, available, options, pairs_with, track_inventory, stock_quantity, low_stock_threshold, auto_hide_out_of_stock")
  ...
  // ---------- Smart Pairing state ----------
  type CategoryFlow = { trigger_category: string; steps: string[] };
  const [categoryFlows, setCategoryFlows] = useState<CategoryFlow[]>([]);

  type PairingStage = string; // Was previously a hardcoded union of stages
  ...
  const getPairingTitle = (stage: string) => {
    const titles: Record<string, { emoji: string; title: string; subtitle: string }> = {
    ...
    return titles[stage.toLowerCase()] || { 
      emoji: "✨", 
      title: `Add ${stage}?`, 
      subtitle: "Would you like to add anything from this category?" 
    };
  };

  const currentPairingInfo = pairingStage ? getPairingTitle(pairingStage) : null;
  ...
  return (
    ...
    {activeOrder && activeOrder.status !== 'cancelled' && activeOrder.status !== 'archived' && viewTracker ? (
      <div className="min-h-screen bg-background">
        ...
        <main className="max-w-2xl mx-auto px-4 py-6 space-y-6 pb-32 animate-in fade-in duration-700">
          ...
          {/* Order Status Card */}
          <div className={`border rounded-[2.5rem] p-8 overflow-hidden relative transition-all duration-700 bg-card border-border shadow-elevated ${
            activeOrder.status === 'pending' 
              ? 'bg-destructive/10 border-destructive shadow-[0_0_50px_-12px_rgba(239,68,68,0.25)] animate-pulse-soft' 
              : ''
          }`}>
            <div className="absolute top-0 right-0 p-4 sm:p-6">
              <div className={`text-[9px] sm:text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-[0.2em] shadow-sm ${activeOrder.status === 'pending' ? 'bg-rose-500 text-white shadow-md shadow-rose-500/20' : 'bg-primary text-primary-foreground shadow-md shadow-primary/20'}`}>
                #{activeOrder.short_code.split('-').pop()}
              </div>
            </div>
            
            <div className="mb-8 pr-12 sm:pr-0">
              <h2 className={`font-display text-xl sm:text-2xl font-black leading-tight tracking-tight flex items-center gap-2 ${activeOrder.status === 'pending' ? 'text-rose-500' : 'text-foreground'}`}>
                {activeOrder.status === 'pending' ? (
                  <>
              ...
              <div className="mt-6 flex flex-wrap gap-2.5">
                <Button size="sm" variant="outline" className="rounded-xl h-10 border-border hover:bg-secondary text-foreground font-bold px-4 transition-all" onClick={() => { setConfirmAction("waiter"); confirmRequest(); }}>
                  <Bell className="h-4 w-4 mr-2 text-primary animate-bounce" /> Call Waiter
                </Button>
                <Button size="sm" variant="outline" className="rounded-xl h-10 border-border hover:bg-secondary text-foreground font-bold px-4 transition-all" onClick={() => { setFeedbackKind("complaint"); setFeedbackOpen(true); }}>
                  <MessageSquareWarning className="h-4 w-4 mr-2 text-rose-500" /> Report Issue
                </Button>
                <Button size="sm" variant="outline" className="rounded-xl h-10 bg-primary/10 border-primary/20 text-primary hover:bg-primary/20 font-bold px-4 transition-all" onClick={() => setViewTracker(false)}>
            ...
            {activeOrder.status === 'pending' && (
              <div className="mb-8 flex justify-center sm:justify-start">
                <Button 
                  ...
                  size="sm" 
                  onClick={cancelOrder}
                  className="h-9 rounded-xl bg-rose-500/10 border-rose-500/20 text-rose-500 hover:bg-rose-500 hover:text-white transition-all gap-2 px-4 font-black text-[10px] uppercase tracking-widest"
                >
            ...
            <div className="relative flex justify-between px-2 mb-2">
              ...
              <div 
                className="absolute top-5 left-8 h-[3px] bg-primary transition-all duration-1000 ease-in-out -z-10 shadow-[0_0_8px_rgba(var(--primary),0.3)]" 
                style={{ width: activeOrder.status === 'pending' ? '0%' : activeOrder.status === 'preparing' ? '45%' : '90%' }}
              />
              
              {[
                { s: 'pending', l: 'Received', icon: Check },
                { s: 'preparing', l: 'Cooking', icon: UtensilsCrossed },
                ...
                return (
                  <div key={step.s} className="flex flex-col items-center gap-3">
                    <div className={`h-11 w-11 rounded-2xl grid place-items-center transition-all duration-700 border ${
                      isDone || isCurrent 
                        ? "bg-primary text-primary-foreground border-primary shadow-glow" 
                        : "bg-secondary border-border text-muted-foreground"
                    }`}>
                      {isDone ? <Check className="h-5 w-5 stroke-[3] text-emerald-500" /> : <step.icon className={`h-5 w-5 ${isCurrent ? "animate-pulse" : ""}`} />}
                    </div>
                    <span className={`text-[9px] font-black uppercase tracking-widest ${isCurrent ? "text-primary" : "text-muted-foreground opacity-60"}`}>{step.l}</span>
                  </div>
                );
              })}
            </div>
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 glass border-b border-border/60 shadow-sm">
        <div className="px-4 py-3 flex items-center justify-between max-w-2xl mx-auto">
          {restaurant?.website_url ? (
            <a href={restaurant.website_url.startsWith('http') ? restaurant.website_url : `https://${restaurant.website_url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 min-w-0 flex-1 hover:opacity-80 transition-opacity">
              {isEvent ? (
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4 pb-20 overflow-x-clip">
    ...
    <ResponsiveModal 
        ...
        className={isMobile ? "rounded-t-[3rem] max-h-[90vh] h-[85vh] flex flex-col p-6 pb-12 border-none shadow-elevated" : "sm:max-w-xl rounded-3xl p-8 max-h-[92vh] flex flex-col overflow-hidden"}
      >
        <div className="overflow-y-auto flex-1 space-y-6 pr-3 scrollbar-thin scrollbar-thumb-border">
    ...
    >
      {active && (
        <ItemSheet 
          ...
          menu={menu}
          isEvent={isEvent} 
          onAdd={(qty, intent, opt) => handleAddItem(active, qty, intent, opt)} 
          onAddPair={(pairedItem, q, intent) => addToCart(pairedItem, q, intent)}
    ...
    >
      <div className="p-8 overflow-y-auto flex-1 space-y-6">
        ...
        <div className="space-y-4 flex-1 overflow-y-auto pr-3 scrollbar-thin scrollbar-thumb-border">
    ...
    <ResponsiveModal 
      ...
      title={currentPairingInfo ? `${currentPairingInfo.emoji} ${currentPairingInfo.title}` : "Pairing"}
      description={currentPairingInfo ? currentPairingInfo.subtitle : "Suggested items"}
    ...
    {(!activeOrderId || !viewTracker) && (
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-full max-w-xs px-4 animate-in slide-in-from-bottom-8 duration-500">
        <button 
          onClick={() => activeOrderId && cart.length === 0 ? setViewTracker(true) : setCartSheetOpen(true)}
          className="w-full bg-card/95 border border-border shadow-[0_15px_35px_rgba(0,0,0,0.15)] dark:shadow-[0_15px_35px_rgba(0,0,0,0.5)] backdrop-blur-md rounded-3xl py-4 px-6 flex items-center justify-between group active:scale-[0.97] transition-all hover:border-primary/20"
        >
          <div className="flex items-center gap-4 text-left">
            <div className="h-12 w-12 rounded-2xl bg-primary text-primary-foreground grid place-items-center relative shadow-md shadow-primary/20">
              {activeOrderId ? <UtensilsCrossed className="h-6 w-6" /> : <ShoppingBasket className="h-6 w-6" />}
              {cart.length > 0 && (
                <span className="absolute -top-1 -right-1 h-5 w-5 bg-background text-primary rounded-full border-2 border-primary text-[10px] font-black grid place-items-center animate-in zoom-in duration-300">
                  {cart.reduce((s, i) => s + (i.qty || 0), 0)}
                </span>
              )}
              {activeOrderId && cart.length === 0 && activeOrder?.status === 'pending' && (
                <span className="absolute -top-1 -right-1 h-3.5 w-3.5 bg-rose-500 rounded-full border-2 border-background animate-pulse" />
              )}
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-[9px] font-black uppercase tracking-[0.2em] text-primary mb-0.5 truncate">
                {activeOrderId 
                  ? (cart.length > 0 ? "Adding to Order" : "Live Tracker") 
                  : (cart.length > 0 ? "Your Tray" : "Tray is Empty")}
              </div>
              <div className="font-display font-black text-sm truncate text-foreground">
                {cart.length > 0 
...
const ItemSheet = ({ 
  ...
  menu: MenuItem[];
  isEvent?: boolean; 
  onAdd: (qty: number, intent: ItemIntent, opt?: string) => void; 
  onAddPair: (m: MenuItem, qty: number, intent: ItemIntent) => void;
  onClose: () => void 
}) => {
  ...
  const [checkedPairs, setCheckedPairs] = useState<Record<string, boolean>>({});
  ...
  // EXCEPTION: if the item has options but no paired items to drive the pairing flow,
  // we must show the options picker so the user can select and the cart doesn't submit empty.
  ...
  const pairedItems = menu.filter((m) => item.pairs_with?.includes(m.id) && m.available);

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        {item.image && (
          <div className="aspect-video w-full rounded-2xl overflow-hidden shadow-soft border border-border/50 bg-secondary">
        ...
        <div className="space-y-1">
          <div className="flex items-start justify-between gap-4">
            <h2 className="font-display text-2xl font-black tracking-tight leading-tight text-foreground">{item.name}</h2>
            {!isEvent && (
              <div className="text-xl font-bold text-primary shrink-0">{formatNaira(Number(item.price))}</div>
            )}
          </div>
          {item.description && (
            <p className="text-sm text-muted-foreground font-semibold leading-relaxed">{item.description}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {item.options!.map((opt) => (
                <button
                  ...
                  className={`px-4 py-3 rounded-xl text-xs font-bold border-2 transition-all ${selected === opt ? "border-primary bg-primary/5 text-primary" : "border-border hover:border-primary/20 text-muted-foreground"}`}
                >
                  {opt}
                </button>
              ))}
        ...
        {pairedItems.length > 0 && (
          <div className="space-y-3">
            <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1.5">
              <Sparkles className="h-3.5 w-3.5 text-emerald-500 animate-pulse" /> Frequently Paired With
            </div>
            <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
              {pairedItems.map((pair) => {
                const isChecked = !!checkedPairs[pair.id];
                return (
                  <div
                    key={pair.id}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="h-10 w-10 bg-secondary rounded-lg overflow-hidden shrink-0 border border-border/50">
                        {pair.image ? (
                          <img src={pair.image} alt={pair.name} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full grid place-items-center text-muted-foreground"><UtensilsCrossed className="h-5 w-5" /></div>
                        )}
                      </div>
                      <div className="min-w-0">
                        <div className="font-bold text-xs text-foreground truncate">{pair.name}</div>
                        <div className="text-[10px] text-muted-foreground font-medium truncate mt-0.5">{pair.category}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2.5 shrink-0 pl-2">
                      {!isEvent && (
                        <span className="text-xs font-black text-emerald-500 dark:text-emerald-400">
                          +{formatNaira(Number(pair.price))}
                        </span>
                      )}
        ...
        <div className="flex items-center justify-between bg-secondary/30 p-4 rounded-2xl border border-border/50">
          ...
          <div className="flex items-center gap-4 bg-background rounded-xl p-1.5 border border-border/30 shadow-sm">
            <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground" onClick={() => { const n = Math.max(1, qty - 1); setQty(n); setQtyInput(String(n)); }}><Minus className="h-4 w-4 stroke-[3]" /></Button>
            <input
              type="number"
              min={1}
              max={999}
            ...
            />
            <Button size="icon" variant="ghost" className="h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground" onClick={() => { const n = Math.min(999, qty + 1); setQty(n); setQtyInput(String(n)); }}><Plus className="h-4 w-4 stroke-[3]" /></Button>
          </div>
        </div>
      </div>
      ...
      <Button 
        ...
        onClick={() => {
          ...
          // For pairing-category items with options hidden by the pairing flow,
          // auto-select the first option so the RPC never receives an empty string.
          const resolvedOption = selected || (hasOptions && item.options![0]) || undefined;
          onAdd(finalQty, itemIntent, resolvedOption);
          
          // Add checked pairings
          pairedItems.forEach((pair) => {
            if (checkedPairs[pair.id]) {
              onAddPair(pair, 1, itemIntent);
            }
          });
      ...
      >
        ADD TO CART {isEvent ? "" : `· ${formatNaira((Number(item.price) * (parseInt(qtyInput) || 1)) + pairedItems.reduce((acc, pair) => acc + (checkedPairs[pair.id] ? Number(pair.price) : 0), 0))}`}
      </Button>
    </div>
  );
};
